...
#ifndef CBAG_POLYGON_POLYGON_45_DATA_H
#define CBAG_POLYGON_POLYGON_45_DATA_H
...
template <typename T> class polygon_45_data : public polygon_data_base<T, polygon_45_data<T>> {
  private:
    using base_cls = polygon_data_base<T, polygon_45_data<T>>;

  public:
    using base_cls::base_cls;
};

template <typename T> struct tag<polygon_45_data<T>> { using type = polygon_45_tag; };

template <typename T> struct polygon_45_traits<polygon_45_data<T>> {
    using polygon_type = polygon_45_data<T>;
